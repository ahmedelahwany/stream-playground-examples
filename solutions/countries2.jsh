...
//1.	Returns whether there is at least one country with the word island in its name ignoring case.
    ...
    boolean IsThereCountryWithIsland= countries.stream().anyMatch(country -> country.getName().toLowerCase().contains("island"));

//2.	Returns the first country name that contains the word island ignoring case.

String CountryWithIsland = countries.stream().
...
IntSummaryStatistics stt = countries.stream().
            mapToInt(country -> (int)country.getTranslations().keySet().stream().count()).
            summaryStatistics();

//7.	Prints the names of countries in the ascending order of the number of timezones.

    countries.stream().
              sorted((o1, o2) -> (int)o1.getTimezones().stream().count() - (int)o2.getTimezones().stream().count()).
              map(Country::getName).
              forEach(System.out::println);
...
//8.	Prints the number of timezones for each country in the form name: population, in the ascending order of the number of timezones.
         ...
         countries.stream().
                    sorted((o1, o2) -> (int)o1.getTimezones().stream().count() - (int)o2.getTimezones().stream().count()).
                    map(country -> country.getName()+" : " + country.getTimezones().stream().count()).
                    forEach(System.out::println);
...
//9.	Returns the number of countries with no Spanish country name translation (the Spanish language is identified by the language code es).
 ...
 long getNumberOfCountriesWithoutES = countries.stream().
             filter(country -> !country.getTranslations().keySet().stream().collect(Collectors.toList()).contains("es")).
            count();
...
double countriesNameAverage = (double)countries.stream().
            mapToDouble(country ->(double) country.getName().length()).
            average().
             getAsDouble();
...
//14.	Returns the largest country with non-null area.
 ...
 Country theLargestCountry = countries.stream().
            filter(country -> country.getArea() != null).
